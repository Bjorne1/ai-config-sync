...
class GlobalRulePage(QWidget):
    ...
    def _build_profile_list_card(self) -> QWidget:
        card = CardFrame("规则版本", "支持新建、复制、删除和搜索。")
        toolbar = QHBoxLayout()
    ...
    def _build_editor_card(self) -> QWidget:
        card = CardFrame("版本编辑", "规则正文按 Markdown 纯文本保存。")
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("规则版本名称")
        self.name_input.textChanged.connect(self._handle_name_changed)
        self.content_editor = QPlainTextEdit()
        self.content_editor.textChanged.connect(self._handle_content_changed)
        action_row = QHBoxLayout()
        action_row.setContentsMargins(0, 0, 0, 0)
        action_row.setSpacing(8)
        self.save_profiles_button = ActionButton("保存版本", "primary")
        self.revert_profiles_button = ActionButton("放弃修改", "secondary")
        self.save_profiles_button.clicked.connect(self._emit_save_profiles)
        self.revert_profiles_button.clicked.connect(self._revert_profiles)
        action_row.addWidget(self.save_profiles_button)
        action_row.addWidget(self.revert_profiles_button)
        action_row.addStretch(1)
        self.editor_meta = QLabel("--")
        self.editor_meta.setObjectName("muted")
        self.editor_meta.setWordWrap(True)
        card.body_layout.addWidget(self.name_input)
        card.body_layout.addWidget(self.content_editor, 1)
        card.body_layout.addLayout(action_row)
        card.body_layout.addWidget(self.editor_meta)
        return card
    ...
    def set_busy(
        ...
        self.save_profiles_button.set_busy(profiles_busy)
        ...
        editing_busy = refresh_busy or profiles_busy or assignments_busy or sync_busy
        self.new_button.setDisabled(editing_busy)
        self.copy_button.setDisabled(editing_busy)
        self.delete_button.setDisabled(editing_busy)
        self.search_input.setDisabled(editing_busy)
        self.profile_list.setDisabled(editing_busy)
        self.name_input.setDisabled(refresh_busy or assignments_busy or sync_busy)
        self.content_editor.setDisabled(refresh_busy or assignments_busy or sync_busy)
        self.revert_profiles_button.setDisabled(editing_busy)
        for card in self._target_cards.values():
            card.set_busy(refresh_busy or profiles_busy or assignments_busy or sync_busy)
        self._refresh_dirty_state()
    ...
    def _select_profile(self, profile_id: str | None) -> None:
        ...
        for index in range(self.profile_list.count()):
            ...
            if item.data(Qt.ItemDataRole.UserRole) == profile_id:
                ...
                return
        self._fill_editor(self._current_profile())
    ...
    def _fill_editor(self, profile: dict[str, object] | None) -> None:
        self._updating_editor = True
        if not profile:
            self.name_input.setText("")
            self.content_editor.setPlainText("")
            self.name_input.setDisabled(True)
            self.content_editor.setDisabled(True)
            self.editor_meta.setText("请先创建或选择一个规则版本。")
        else:
            self.name_input.setDisabled(False)
            self.content_editor.setDisabled(False)
            self.name_input.setText(str(profile.get("name") or ""))
            self.content_editor.setPlainText(str(profile.get("content") or ""))
            updated_at = str(profile.get("updatedAt") or "未保存")
            file_name = str(profile.get("file") or f"{profile['id']}.md")
            self.editor_meta.setText(
                f"文件：{file_name}  ·  更新时间：{updated_at}"
            )
        self._updating_editor = False

    def _handle_name_changed(self, value: str) -> None:
        ...
        if not profile:
            return
        profile["name"] = value
        current_id = str(profile["id"])
        self._refresh_profile_list()
        self._select_profile(current_id)
        self._refresh_target_cards()
        self._refresh_dirty_state()

    def _handle_content_changed(self) -> None:
        if self._updating_editor:
            return
        profile = self._current_profile()
        if not profile:
            return
        profile["content"] = self.content_editor.toPlainText()
        self._refresh_target_cards()
        self._refresh_dirty_state()
    ...
    def _create_profile(self) -> None:
        ...
        self._profiles.append(
            {
                "id": profile_id,
                "name": f"全局规则 {len(self._profiles) + 1}",
    ...
    def _emit_save_profiles(self) -> None:
        if any(not str(profile.get("name") or "").strip() for profile in self._profiles):
            QMessageBox.warning(self, "保存失败", "规则版本名称不能为空。")
            return
        payload = {
            "profiles": [
                {
    ...
    def _revert_profiles(self) -> None:
        self._profiles = deepcopy(self._original_profiles)
        current_ids = {profile["id"] for profile in self._profiles}
        if self._selected_profile_id not in current_ids:
            self._selected_profile_id = self._profiles[0]["id"] if self._profiles else None
        self._refresh_profile_list()
        self._select_profile(self._selected_profile_id)
        self._refresh_target_cards()
    ...
    def _refresh_target_cards(self) -> None:
        sync_enabled = not self._profiles_dirty() and not self._assignments_dirty()
    ...
    def _display_status(self, environment_id: str, tool_id: str) -> dict[str, object]:
        ...
        if self._profile_changed(profile_id):
            return {
                **status,
                "state": "outdated",
                "message": "规则版本有未保存修改",
    ...
    def _refresh_dirty_state(self) -> None:
        profiles_dirty = self._profiles_dirty()
        ...
        blocked_reason = []
        if profiles_dirty:
            blocked_reason.append("规则版本有未保存修改")
        if assignments_dirty:
            blocked_reason.append("目标映射有未保存修改")
        sync_message = "；".join(blocked_reason) if blocked_reason else "可直接同步"
        ...
        self.sync_selected_button.setDisabled(bool(blocked_reason))
        self.sync_all_button.setDisabled(bool(blocked_reason))

    def _profiles_dirty(self) -> bool:
        return serialize(self._profile_payload(self._profiles)) != serialize(
            self._profile_payload(self._original_profiles)
        )

    def _assignments_dirty(self) -> bool:
        return serialize(self._assignments) != serialize(self._original_assignments)

    def _profile_payload(self, profiles: list[dict[str, object]]) -> list[dict[str, str]]:
        return [
            {
                "id": str(profile["id"]),
                "name": str(profile.get("name") or ""),
    ...
    def _profile_changed(self, profile_id: str) -> bool:
        current = self._profile_by_id(profile_id)
        original = next(
            (
                profile
                for profile in self._original_profiles
        ...
        if current is None or original is None:
            return True
        return serialize(self._profile_payload([current])) != serialize(
            self._profile_payload([original])
        )
