...
- Framework: Strictly adhere to the existing project's tech stack (e.g., Vue, Svelte, Angular). Only default to Next.js / React for entirely new projects from scratch.
...
- Path separator: always use backslash `\` for file paths. If ≥ 3 consecutive failures occur, fall back to Node.js for the operation.
