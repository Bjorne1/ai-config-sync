...
- **Never commit directly to `main`**: If the current project has a git repo and the active branch is `main` (or `master`), always create and switch to a new feature/fix branch before making any code changes. Use descriptive branch names (e.g. `feat/add-login`, `fix/null-pointer`).
