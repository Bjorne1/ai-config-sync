...
function resolveSourceDir(configPath) {
  ...
  // 如果是相对路径，基于项目根目录解析（而非 cwd）
  if (!path.isAbsolute(configPath)) {
    return path.resolve(PROJECT_ROOT, configPath);
  }
  // 如果是绝对路径但不存在（可能是跨平台问题），尝试基于项目根目录 fallback
  if (!fs.existsSync(configPath)) {
    const basename = path.basename(configPath);
    const fallback = path.resolve(PROJECT_ROOT, basename);
...
function initConfig() {
  ...
  // 创建源目录
  const resolvedSourceDir = resolveSourceDir(defaultConfig.sourceDir);
  if (!fs.existsSync(resolvedSourceDir)) {
    fs.mkdirSync(resolvedSourceDir, { recursive: true });
  }

  // 创建 commands 源目录
  const resolvedCommandsDir = resolveSourceDir(defaultConfig.commandsSourceDir);
  if (!fs.existsSync(resolvedCommandsDir)) {
    fs.mkdirSync(resolvedCommandsDir, { recursive: true });
  }
